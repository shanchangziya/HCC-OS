...
Idents(sce.noNE) <- "final_celltype"

# 进行细胞抽样（每个细胞类型保留30%，减少计算负担）
cat("对单细胞数据进行抽样（保留30%细胞）...\n")
...
cat("抽样完成，各细胞类型数量：\n")
print(table(sce.noNE.30$final_celltype))
...
sc.dat <- ref
cell.type.labels  <- sce.noNE.30$final_celltype
cell.state.labels <- sce.noNE.30$final_celltype  # 使用相同的细胞类型标签
...
plot.cor.phi(input=ref,
             ...
             pdf.prefix="TCGA-LIHC_cell.type.correlation",
...
plot.cor.phi(input=ref,
             ...
             pdf.prefix="TCGA-LIHC_cell.state.correlation",
...
plot.bulk.vs.sc(
  ...
  bulk.input=bk.dat,
  pdf.prefix="TCGA-LIHC_bulk.vs.sc"
)
...
# 保存完整结果
cat("保存完整BayesPrism结果到 TCGA-LIHC_bp.res.Rdata...\n")
save(bp.res, file="TCGA-LIHC_bp.res.Rdata")

# 保存细胞类型比例矩阵
cat("保存细胞类型比例矩阵到 TCGA-LIHC_cell_fractions.csv...\n")
write.csv(theta, file="TCGA-LIHC_cell_fractions.csv", row.names=TRUE)
...
cat("保存合并数据到 TCGA-LIHC_deconv_with_survival.csv...\n")
write.csv(merged_data, file="TCGA-LIHC_deconv_with_survival.csv", row.names=FALSE)
...
# 细胞类型比例箱线图
pdf("TCGA-LIHC_cell_fraction_boxplot.pdf", width=12, height=6)
theta_long <- reshape2::melt(theta)
colnames(theta_long) <- c("Sample", "CellType", "Fraction")
p1 <- ggplot(theta_long, aes(x=CellType, y=Fraction, fill=CellType)) +
  geom_boxplot() +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45, hjust=1, size=8),
        legend.position="none") +
  labs(title="TCGA-LIHC Cell Type Fractions",
       x="Cell Type",
       y="Fraction")
print(p1)
dev.off()
...
# 细胞类型比例热图（前50个样本）
pdf("TCGA-LIHC_cell_fraction_heatmap.pdf", width=10, height=8)
library(pheatmap)
pheatmap(t(theta[1:min(50, nrow(theta)),]),
         cluster_rows=TRUE,
         cluster_cols=TRUE,
         show_colnames=FALSE,
         main="TCGA-LIHC Cell Type Fractions (Top 50 samples)")
dev.off()

cat("\n=== 全部完成！===\n")
cat("输出文件：\n")
cat("  1. TCGA-LIHC_bp.res.Rdata - 完整BayesPrism结果对象\n")
cat("  2. TCGA-LIHC_cell_fractions.csv - 细胞类型比例矩阵\n")
cat("  3. TCGA-LIHC_deconv_with_survival.csv - 合并生存数据的结果\n")
cat("  4. TCGA-LIHC_cell_fraction_boxplot.pdf - 细胞类型比例箱线图\n")
cat("  5. TCGA-LIHC_cell_fraction_heatmap.pdf - 细胞类型比例热图\n")
cat("  6. TCGA-LIHC_*.pdf - 各种质量控制图\n")
